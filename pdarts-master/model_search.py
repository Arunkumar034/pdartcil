...
class Network(nn.Module):
    ...
    def __init__(self, C, num_classes, layers, criterion, steps=4, multiplier=4, stem_multiplier=3,
                 switches_normal=[], switches_reduce=[], p=0.0, in_channels=3):
        """
        Args:
            ...
            steps: number of nodes inside a cell (DARTS uses 4)
            multiplier: output channels multiplier (concatenated nodes)
            stem_multiplier: multiplier for stem output channels
            switches_normal: list of boolean lists indicating which operations are active on each edge of normal cells
            switches_reduce: same for reduction cells
            p: dropout probability applied to skip‑connect operations
        ...
        # ----- MODIFIED: use in_channels instead of hardcoded 3 -----
